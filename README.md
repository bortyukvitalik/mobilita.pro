...
│
├── site/
│   ├── index.html             ← лендинг (один файл), формы → /api/leads
│   ├── privacy.html           ← политика 152-ФЗ (шаблон, поля [ЗАПОЛНИТЬ])
...
- **Заполнить `site/privacy.html`** — реквизиты оператора ПДн (поля `[ЗАПОЛНИТЬ]`),
  юрист. Регистрация оператора ПДн в Роскомнадзоре.
